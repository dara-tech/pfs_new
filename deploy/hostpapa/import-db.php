<?php
/**
 * Import schema.sql from same directory (upload first). One-time setup.
 */
...
if (!is_file($sqlFile)) {
    exit("Upload schema.sql next to this script first.\n");
}

$envPath = '/home/nchads3/psf-backend/psf-backend/.env';
if (!is_file($envPath)) {
    exit("Missing $envPath\n");
}
...
if ($mysqli->error) {
    exit('Import error: ' . $mysqli->error . "\n");
}
